...
// Subcommands:
//
//	serve     run the daemon on this host (M0: config load + /healthz only)
...
func cmdServe(args []string) {
	...
	addr := fs.String("addr", ":7433", "daemon listen address")
	fs.Parse(args)
	...
	log.Printf("hotlane %s serving app %q on %s (ring=%d)", version, cfg.App, *addr, cfg.Ring)
